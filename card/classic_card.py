...
class MindControlTech(MinionNoPoint):
    ...
    def utilize_delta_h_and_arg(cls, state, game_state, hand_card_index):
        if game_state.oppo_state.oppo_minion_num < 4:
            return cls.value, game_state.my_state.my_minion_num
        else:
            h_sum = sum([minion.heuristic_val for minion in game_state.oppo_state.oppo_minions])
            h_sum /= game_state.oppo_state.oppo_minion_num
...
class EarthenRingFarseer(MinionPointMine):
    @classmethod
    def utilize_delta_h_and_arg(cls, state, game_state, hand_card_index):
        best_h = 0.2 + game_state.my_state.my_hero.delta_h_after_heal(3)
        if game_state.my_state.my_hero.health <= 5:
            best_h += 4
        best_my_index = -1
        ...
        return best_h, game_state.my_state.my_minion_num, best_my_index
...
class DoomSayer(MinionNoPoint):
    ...
    def utilize_delta_h_and_arg(cls, state, game_state, hand_card_index):
        # 一费别跳末日
        if game_state.my_state.my_total_mana == 1:
            return 0,

        # 二三费压末日就完事了
        if game_state.my_state.my_total_mana <= 3:
            return 1000,

        # 优势不能上末日
        if game_state.my_state.my_heuristic_value >= game_state.oppo_state.oppo_heuristic_value:
            return 0,
        ...
        if oppo_attack_sum >= 7:
            ...
            return game_state.oppo_state.oppo_heuristic_value - game_state.my_state.my_heuristic_value,
