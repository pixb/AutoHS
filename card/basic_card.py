...
class MinionCard(Card):
    ...
    def utilize_delta_h_and_arg(cls, state, game_state, hand_card_index):
        if cls.value != 0:
            ...
            if game_state.my_state.my_hero.health <= 10 and hand_card.taunt:
                delta_h *= 1.5

            return delta_h, game_state.my_state.my_minion_num  # 默认放到最右边
    ...
    def best_h_and_arg(cls, state, game_state, hand_card_index):
        ...
        if len(args) == 0:
            args = [game_state.my_state.my_minion_num]
...
class Coin(SpellNoPoint):
    ...
    def best_h_and_arg(cls, state, game_state, hand_card_index):
        ...
        for another_index, hand_card in enumerate(game_state.my_state.my_hand_cards):
            ...
            if hand_card.current_cost != game_state.my_state.my_last_mana + 1:
                continue
